...
- (void)initProperties {
    ...
    _edit = cee_ascii_edit_create((__bridge cee_pointer)self);
    CEESize size = cee_size_make(self.frame.size.width,
                                 self.frame.size.height);
    cee_ascii_edit_container_size_set(_edit, size);
}

- (void)setTextAttributesDescriptor:(NSString*)descriptor {
    cee_ascii_edit_attributes_configure(_edit, (const cee_uchar*)[descriptor UTF8String]);
    [self setNeedsDisplay:YES];
}

- (void)dealloc {
    if (_edit) {
        cee_ascii_edit_free(_edit);
        _edit = NULL;
    }
}
...
- (void)setFrameSize:(NSSize)newSize {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(ASCIIViewFrameChanged:)])
        [_delegate ASCIIViewFrameChanged:self];
}
...
- (void)drawUnderlineAtUnit:(CEEASCIIUnitRef)unit inLine:(CEEASCIILineRef)line {
    NSBezierPath* path = nil;
    CEERect line_bounds;
    CEEASCIILayoutRef layout = NULL;
    CGPoint position;
    cee_float ascent = 0.0;
    CEERect unit_bounds;
    CGFloat base = 0.0;
    
    layout = cee_ascii_edit_layout_get(_edit);
    if (!layout)
        return;

    base = cee_ascii_line_base_get(line);
    line_bounds = cee_ascii_line_bounds_get(line);
    unit_bounds = cee_ascii_unit_bounds_get(unit);
    ascent = cee_ascii_unit_ascent_get(unit);
    position = (CGPoint){
...
- (void)drawRect:(NSRect)dirtyRect {
    ...
    CEEASCIILineRef line = NULL;
    CGFloat base = 0.0;
    CEEList* p = NULL;
    CEEList* q = NULL;
    CEEASCIIUnitRef unit = NULL;
    CGGlyph glyph = 0;
    CTFontRef font = NULL;
    ...
    platform = cee_ascii_layout_platform_get(layout);
    ...
    p = cee_ascii_layout_lines_get(layout);
    ...
    while (p) {
        ...
        q = cee_ascii_line_units_get(line);
        while (q) {
            unit = q->data;
            glyph = (CGGlyph)cee_ascii_unit_glyph_get(unit);
            font = (CTFontRef)cee_ascii_unit_font_get(unit);
            fore_color = (CGColorRef)cee_ascii_unit_fore_color_get(unit);
            background_color = (CGColorRef)cee_ascii_unit_background_color_get(unit);
            unit_bounds = cee_ascii_unit_bounds_get(unit);
            position = (CGPoint){
                line_bounds.origin.x + unit_bounds.origin.x,
                base
            ...
            ascent = cee_ascii_unit_ascent_get(unit);
            under_line = cee_ascii_unit_under_line_get(unit);
            position = [self viewPointFromLayoutPoint:position];
...
- (void)scrollLineUp:(id)sender {
    cee_ascii_edit_scroll_line_up(_edit);
    [self setNeedsDisplay:YES];
    if (_delegate && [_delegate respondsToSelector:@selector(ASCIIViewScrolled:)])
        [_delegate ASCIIViewScrolled:self];
}

- (void)scrollLineDown:(id)sender {
    cee_ascii_edit_scroll_line_down(_edit);
    [self setNeedsDisplay:YES];
    if (_delegate && [_delegate respondsToSelector:@selector(ASCIIViewScrolled:)])
        [_delegate ASCIIViewScrolled:self];
}
